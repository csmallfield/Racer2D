...
func _car_steer(car: Dictionary, car_seg: Dictionary, player_seg: Dictionary,
	...
	for i in range(1, AI_LOOKAHEAD):
		...
		if seg.index == player_seg.index and car.speed > player.speed 				and _overlap(player.x, player_w, car.offset, car_w, 1.2):
			var dir := 0.0
			if player.x > 0.5:
				dir = -1.0
			elif player.x < -0.5:
				...
				dir = 1.0 if float(car.offset) > player.x else -1.0
		...
		for other in seg.cars:
			if other == car:
				continue
			var other_w: float = SpriteCatalog.get_def(other.sprite).world_w 					/ RoadRenderer.ROAD_WIDTH
			if car.speed > other.speed 					and _overlap(car.offset, car_w, other.offset, other_w, 1.2):
				var dir := 0.0
				if float(other.offset) > 0.5:
					dir = -1.0
				elif float(other.offset) < -0.5:
					dir = 1.0
				else:
					dir = 1.0 if float(car.offset) > float(other.offset) else -1.0
